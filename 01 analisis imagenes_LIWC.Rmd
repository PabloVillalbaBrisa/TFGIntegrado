...
subtitle: "Datos: tenistas + posts + imágenes + LIWC"
...
knit: (function(inputFile, encoding) { 
    rmarkdown::render(
        ...
        output_file = 'C:/Users/pvill/OneDrive/Documentos/TFG INTEGRADO/Resultados_R/IG-ATP imágenes_LIWC.tex') 
    })
---
...
library(pander)       # tablas en rmarkdown
library(gridExtra)
```
...
# Comparaciones de modelos de likes

```{r}
likes_no_image_sin_LIWC <- likes.log ~ Puntos.sqrt + Edad + Nike + Wilson + Yonex + Adidas + Asics + Head + publicaciones.log + followers.log + following.log + weekend + Australian_Open + Roland_Garros + Wimbledon + US_Open + franja_horaria + video + is_sponsored + num_hashtags + num_mentions + num_emojis + word_count.sqrt + (1|username)

# evaluamos el modelo
modelo.likes_no_image_sin_LIWC <- lmer(likes_no_image_sin_LIWC, data=datos, REML=FALSE)

likes_image_sin_LIWC <- likes.log ~ Puntos.sqrt + Edad + Nike + Wilson + Yonex + Adidas + Asics + Head + publicaciones.log + followers.log + following.log + weekend + Australian_Open + Roland_Garros + Wimbledon + US_Open + franja_horaria + video + is_sponsored + num_hashtags + num_mentions + num_emojis + word_count.sqrt + texto.detectado + tennis.instruments + rostros.sqrt + emocion + (1|username)

# evaluamos el modelo
modelo.likes_image_sin_LIWC <- lmer(likes_image_sin_LIWC, data=datos, REML=FALSE)

# tabla
tab_model(modelo.likes_no_image_sin_LIWC, modelo.likes_image_sin_LIWC, modelo.likes, title = 'Resultados de Comparación de modelos para likes', dv.labels = c('Sin Imágenes', 'Con Imágenes','Con Imágenes + Texto'))

tab_df(anova(modelo.likes_no_image_sin_LIWC, modelo.likes_image_sin_LIWC), title = 'ANOVA entre ambos modelos sin y con variables de imagen', show.rownames =T)

tab_df(anova(modelo.likes_image_sin_LIWC, modelo.likes), title = 'ANOVA entre ambos modelos sin y con LIWC', show.rownames =T)
```

# Comparaciones de modelos de comentarios

```{r}
comentarios_no_image_sin_LIWC <- comentarios.log ~ Puntos.sqrt + Edad + Nike + Wilson + Yonex + Adidas + Asics + Head + publicaciones.log + followers.log + following.log + weekend + Australian_Open + Roland_Garros + Wimbledon + US_Open + franja_horaria + video + is_sponsored + num_hashtags + num_mentions + num_emojis + word_count.sqrt + (1|username)

# evaluamos el modelo
modelo.comentarios_no_image_sin_LIWC <- lmer(comentarios_no_image_sin_LIWC, data=datos, REML=FALSE)

comentarios_image_sin_LIWC <- comentarios.log ~ Puntos.sqrt + Edad + Nike + Wilson + Yonex + Adidas + Asics + Head + publicaciones.log + followers.log + following.log + weekend + Australian_Open + Roland_Garros + Wimbledon + US_Open + franja_horaria + video + is_sponsored + num_hashtags + num_mentions + num_emojis + word_count.sqrt + texto.detectado + tennis.instruments + rostros.sqrt + emocion + (1|username)

# evaluamos el modelo
modelo.comentarios_image_sin_LIWC <- lmer(comentarios_image_sin_LIWC, data=datos, REML=FALSE)

# tabla
tab_model(modelo.comentarios_no_image_sin_LIWC, modelo.comentarios_image_sin_LIWC, modelo.comentarios, title = 'Resultados de Comparación de modelos para likes', dv.labels = c('Sin Imágenes', 'Con Imágenes','Con Imágenes + Texto'))

tab_df(anova(modelo.comentarios_no_image_sin_LIWC, modelo.comentarios_image_sin_LIWC), title = 'ANOVA entre ambos modelos sin y con variables de imagen', show.rownames =T)

tab_df(anova(modelo.comentarios_image_sin_LIWC, modelo.comentarios), title = 'ANOVA entre ambos modelos sin y con LIWC', show.rownames =T)
```